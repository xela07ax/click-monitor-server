...
func NewWsLogger(execInputMsg func([]byte)[]byte) *WsLogger {
	return &WsLogger{
		...
		// логер лучше не передавать во избежания зацикливания
	}
}
...
func (hub *WsLogger) ServeWs(w http.ResponseWriter, r *http.Request) {
	...
	client := &Client{hub: hub, conn: conn, send: make(chan []byte, 256), Loger: nil}
	fmt.Println("way2")
	client.hub.register <- client
	fmt.Println("нов клиент")
...
func (hub *WsLogger) HomePageWs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Fatal(fmt.Errorf("err:%v|func:WsLogger.HomePageWs", err))
	}
}
